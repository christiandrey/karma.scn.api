...
export namespace MapUser {
    ...
    export function inUsersControllerGetProfileLiteAsync(user: User): User {
        const { id, type, firstName, lastName, urlToken, verified, profilePhoto, twitterUrl, facebookUrl, linkedInUrl, googlePlusUrl, company } = user;
        return {
            id, type, firstName, lastName, verified, urlToken, twitterUrl, facebookUrl, linkedInUrl, googlePlusUrl,
    ...
    export function inUsersControllerGetProfileAsync(user: User): User {
        const { id, urlToken, address, verified, type, firstName, lastName, twitterUrl, facebookUrl, linkedInUrl, googlePlusUrl, email, phone, cpdPoints, description, experiences, certifications, skills } = user;
        return {
            id, urlToken, verified, type, firstName, lastName, twitterUrl, facebookUrl, linkedInUrl, googlePlusUrl, email, phone, cpdPoints, description,
            latestExperience: !!user.latestExperience && user.latestExperience.current ? {
                role: user.latestExperience.role,
                organization: user.latestExperience.organization
            } : null,
