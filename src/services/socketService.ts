...
export namespace SocketService {
    ...
    export async function addDiscussionComment(req: Request, comment: Comment, urlToken: string): Promise<Comment> {
        const discussionRepository = getRepository(Discussion);
        const discussion = await discussionRepository.findOne({ urlToken });
    ...
    export async function addWebinarComment(req: Request, comment: Comment, urlToken: string): Promise<Comment> {
        const webinarRepository = getRepository(Webinar);
        const webinar = await webinarRepository.findOne({ urlToken });
